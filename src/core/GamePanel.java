...
public class GamePanel extends JPanel implements Runnable, KeyListener {
	...
	private long pauseLength;			// Time game is spent paused
	private long pauseStart;			// Time game started pausing
	 ...
	 */
	public GamePanel(Game game, /*char[][] map,*/ ArrayList<Box> boxes, long period) {
		this.gameTop = game;
		//this.map = map;
		this.boxes = boxes;
		this.period = period;
	 ...
	 */
	public void stopGame(String fromWhere) {
		running = false;
	...
	/**
	 * This method takes cares of all rendering, updating,
	 * and correct FPS.
	 */
	public void run() {
	...
	private void gameUpdate() {
		 ...
		 */
		// TODO: Check if bomber is hit by explosion after it has been
		// drawn. Don know if this goes here or in fillInExplosion().
	...
	private boolean checkValidMove(/*char[][] map,*/ Position dPosition) {
		 ...
		 */
		for (int a = 0; a < bombs.size(); a++)
			if (bombs.get(a).getPosition().compare(dPosition))
				return false;
		...
		for (int i = 0; i < boxes.size(); i++) {
			try {
				if (boxes.get(i).getPosition().compare(dPosition))
					return false;
			}
			catch (NullPointerException e) {
				// Ignore exception because boxes wasn't full
	...
	/**
	 * This updates the bomb fuse lengths based on how
	 * long the game has been paused.
	 * 
	 * @param time The length the game was paused.
	 */
	private void updateBombFuses(long time) {
		for (int p = 0; p < bombs.size(); p++)
			bombs.get(p).addToFuseLength(time);
	} // End updateBombFuses
	...
	public void keyPressed(KeyEvent ke) {		
		...
		switch (keyCode) {
			...
			case 16:	// SHIFT
				break;
			case 27:	// ESCAPE
				stopGame("Closed correctly from GamePanel.");
				break;
			...
			case 80:	// P
				if (!isPaused) {
					...
					isPaused = false;
					pauseLength = System.nanoTime() - pauseStart;
					updateBombFuses(pauseLength);
				}
				break;
